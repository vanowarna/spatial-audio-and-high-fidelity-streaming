  ...
  sections: [
    {
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: '1. Introduction & Motivation', font: 'Arial', size: 22 })],
        }),
        new Paragraph({
          text: '2. System Setup & Methodology',
          spacing: { before: 100, after: 100 },
          children: [new TextRun({ text: '2. System Setup & Methodology', font: 'Arial', size: 22 })],
        }),
        new Paragraph({
          text: '3. Performance Comparison & Results',
          spacing: { before: 100, after: 100 },
          children: [new TextRun({ text: '3. Performance Comparison & Results', font: 'Arial', size: 22 })],
        }),
        new Paragraph({
          text: '4. Discussion',
          spacing: { before: 100, after: 100 },
          children: [new TextRun({ text: '4. Discussion', font: 'Arial', size: 22 })],
        }),
        new Paragraph({
          text: '5. Conclusions',
          spacing: { before: 100, after: 100 },
          children: [new TextRun({ text: '5. Conclusions', font: 'Arial', size: 22 })],
        }),
        new Paragraph({
          text: 'References',
          spacing: { before: 100, after: 100 },
          children: [new TextRun({ text: 'References', font: 'Arial', size: 22 })],
        }),
        ...
        new Paragraph({
          text: 'Comparative codec evaluation: Assess AAC, Opus, and MP3 across bitrates ranging from 24 to 256 kbps using both objective quality metrics and subjective listening tests.',
          bullet: { level: 0 },
          font: 'Arial',
          size: 22,
        ...
        new Paragraph({
          text: 'Spatial audio implementation: Develop and validate a Web Audio API-based spatial audio renderer utilizing HRTF-based binaural processing and 3D scene management via Three.js.',
          bullet: { level: 0 },
          font: 'Arial',
          size: 22,
          spacing: { after: 100, line: 240 },
        ...
        new Paragraph({
          text: 'Transparency threshold identification: Determine the minimum bitrate at which each codec becomes perceptually transparent for spatial audio content, considering both technical quality and immersive presence.',
          bullet: { level: 0 },
          font: 'Arial',
          size: 22,
          spacing: { after: 100, line: 240 },
        ...
        new Paragraph({
          text: 'Network resilience analysis: Evaluate codec performance under realistic network conditions including packet loss (0-30%) and jitter (0-500ms) to assess real-world streaming viability.',
          bullet: { level: 0 },
          font: 'Arial',
          size: 22,
          spacing: { after: 100, line: 240 },
...
function createFigurePlaceholder(text) {
  ...
  return new Table({
    ...
    rows: [new TableRow({ height: { value: 300, rule: 'auto' }, children: [cell] })],
  });
}
...
function createTransparencyTable() {
  ...
  return new Table({
    ...
    rows: [
      new TableRow({
        height: { value: 250, rule: 'auto' },
        children: [
          headerCell('Codec'),
          headerCell('Transparency Bitrate (kbps)'),
          headerCell('Quality Margin at 64 kbps'),
        ],
      }),
      new TableRow({
        height: { value: 200, rule: 'auto' },
        children: [dataCell('MP3'), dataCell('~160'), dataCell('Below transparency')],
      }),
      new TableRow({
        height: { value: 200, rule: 'auto' },
        children: [dataCell('AAC'), dataCell('~96'), dataCell('Moderate degradation')],
      }),
      new TableRow({
        height: { value: 200, rule: 'auto' },
        children: [dataCell('Opus'), dataCell('~64'), dataCell('Transparent/imperceptible')],
      }),
    ],
  });
}
...
function createReference(number, text) {
  return new Paragraph({
    children: [new TextRun({ text: `${number} ${text}`, font: 'Arial', size: 22 })],
    alignment: AlignmentType.JUSTIFIED,
